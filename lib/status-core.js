...
function reconcileActiveSessions(sessions, activeSessionIds) {
  const active = activeSessionIds instanceof Set ? activeSessionIds : new Set(activeSessionIds);
  return sessions.map((session) => {
    if (session.agent !== "codex" || session.status !== "completed" || !active.has(session.sessionId)) return session;
    const next = { ...session, status: "working" };
    delete next.attentionKind;
    return next;
  });
}
